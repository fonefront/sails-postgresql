...
module.exports = (function() {
  ...
  var adapter = {
    ...
    renameAttribute: function(table, oldAttrName, newAttrName, cb) {
      spawnConnection(function __RENAME_ATTRIBUTE__(client, cb) {

          // Escape Names
          table = utils.escapeName(table);
          oldAttrName = utils.escapeName(oldAttrName);
          newAttrName = utils.escapeName(newAttrName);

          // Build Query
          var query = 'ALTER TABLE ' + table + ' RENAME COLUMN ' + oldAttrName + ' TO ' + newAttrName;

          // Run Query
          client.query(query, function __RENAME_ATTRIBUTE__(err, result) {
              if(err) return cb(err);
              cb(null, result.rows);
          });
